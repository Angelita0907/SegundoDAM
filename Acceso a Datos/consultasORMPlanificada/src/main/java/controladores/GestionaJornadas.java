...
public class GestionaJornadas {  
	...
	public static void main(String[] args) {
		...
		// 1. devolver participantes
		List<Object[]> listaParticipantes = participanteDao.getParticipantes();
		for (Object[] fila : listaParticipantes) {
			String nombre = (String) fila[0];
			String apellidos = (String) fila[1];
			String email = (String) fila[2];
			
			logger.info("Particpantes = Nombre: "+nombre+ ", apellidos: "+ apellidos+ ", email: "+email);
		}
		
		// comento las siguientes para poder probar de una en una
		/*
		//2 eventos de mas de 91 minutos
		List<Object[]> listaEventos = eventoDao.eventosMas91Min();
		for (Object[] fila : listaEventos) {
			String nombre = (String) fila[0];
			String tipo = (String) fila[1];
			Integer duracion = (Integer) fila[2];
			
			logger.info("Eventos = Nombre: "+nombre+ ", tipo: "+ tipo+ ", duración: "+duracion);
		}*/
		
		//3 numero participantes de cada evento
		/*
		List<Object[]> particpantesEvento = eventoDao.participantesEvento();
		for (Object[] fila : particpantesEvento) {
			String nombre = (String) fila[0];
			Ubicacion ubicacion = (Ubicacion) fila[1];
			Integer total = (Integer) fila[2];
			logger.info("Eventos = Nombre: "+nombre+ ", ubicacion: "+ ubicacion+ ", participantes: "+total);

		}*/
/*
		//4 eventos por ubicacion
		List<Object[]> eventosUbicacion = eventoDao.listaEventos("Salon de actos");
		for (Object[] fila : eventosUbicacion) {
			String nombre = (String) fila[0];
			String tipo = (String) fila[1];
			LocalDateTime fecha = (LocalDateTime) fila[2];
			logger.info("Eventos = Nombre: "+nombre+ ", tipo: "+ tipo+ ", fecha: "+fecha);
