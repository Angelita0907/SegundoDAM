...
public class Servidor {
	...
	public static void main(String[] args) {
		...
		MapaCompartido mapaVuelos = new MapaCompartido();
		...
		try (ServerSocket servidor = new ServerSocket(puerto)) {
			...
			while (true) {
				...
				System.out.println("Nuevo cliente conectado: " + socketCliente.getInetAddress() + ", en fecha: "
						+ LocalDateTime.now());
				// 2. Lanza un hilo nuevo para este cliente específico
				// Esto permite que el bucle vuelva al accept() inmediatamente
				new HiloServidor(socketCliente, contador, mapaVuelos).start();
			}
		} catch (IOException e) {
